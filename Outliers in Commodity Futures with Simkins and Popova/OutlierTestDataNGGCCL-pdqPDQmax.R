...
#'
#' 10/31/18 updated mysql to fix bug with timezone.  running to get prompts
#' 
    ...
    withCallingHandlers ({
      tryCatch( {
        tmp1 <- tso(ts(tmp[!is.na(tmp)]), tsmethod = c("auto.arima"),
                    remove.method=remove.method, maxit.iloop=maxit.iloop,
...
#                                        tsmethod = c("auto.arima"), remove.method=remove.method, maxit.iloop=20,
#                                        args.tsmethod=list(allowdrift=TRUE, allowmean=TRUE, method='ML'),
#                                        types=c("AO", "LS", "TC","IO")))
# ar2 <- lapply(tmp, FUN=function(x) tso(ts(x[!is.na(x)]),
#                                        tsmethod = c("arima"), remove.method=remove.method, maxit.iloop=20,
...
# tmp1 <- tso(ts(tmp[!is.na(tmp)]), tsmethod = c("auto.arima"),
#             remove.method=remove.method, maxit.iloop=20,
#             args.tsmethod=list(allowdrift=TRUE, allowmean=TRUE),
#             types=c("AO", "LS", "TC","IO"))
#
# tmp1 <- tso(ts(tmp[!is.na(tmp)]), tsmethod = c("arima"),
